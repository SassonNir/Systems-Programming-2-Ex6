...
template <typename T>
struct is_option : ::std::bool_constant<::std::is_same_v<T, name> ||
...
template <typename F>
struct invocable_with_option : ::std::bool_constant<::std::is_invocable_v<F, name> ||
...
template <typename T>
struct has_value_field<T, ::std::void_t<decltype(::std::declval<T>().value)>> : ::std::true_type {};
...
tableprinter::printer::printer(::std::vector<column> columns, ::std::vector<osref> streams)
    : m_columns{move(columns)}, m_streams{move(streams)} {}
...
void tableprinter::printer::print_column(int col, ::std::ostream& os, const H& val, const T&... rest) {
    if constexpr (detail::is_sequence_v<H>) {
        ...
        const auto& options{m_columns[static_cast<size_t>(col++)].options};
